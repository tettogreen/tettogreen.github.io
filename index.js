...
function ___syscall_statfs64(path, size, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 0, 1, path, size, buf);
 try {
  path = SYSCALLS.getStr(path);
  assert(size === 64);
...
function __setitimer_js(which, timeout_ms) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, which, timeout_ms);
 if (timers[which]) {
...
function _emscripten_webgl_destroy_context(contextHandle) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(38, 0, 1, contextHandle);
...
function _emscripten_webgl_create_context_proxied(target, attributes) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 0, 1, target, attributes);
...
function _emscripten_webgl_enable_extension(contextHandle, extension) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(40, 0, 1, contextHandle, extension);
...
function _godot_audio_init(p_mix_rate, p_latency, p_state_change, p_latency_update) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 0, 1, p_mix_rate, p_latency, p_state_change, p_latency_update);
...
function _godot_audio_worklet_create(channels) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(55, 0, 1, channels);
 try {
...
function _godot_audio_worklet_start(p_in_buf, p_in_size, p_out_buf, p_out_size, p_state) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 0, 1, p_in_buf, p_in_size, p_out_buf, p_out_size, p_state);
...
function _godot_js_config_canvas_id_get(p_ptr, p_ptr_max) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(57, 0, 1, p_ptr, p_ptr_max);
...
function _godot_js_display_alert(p_text) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 0, 1, p_text);
...
function _godot_js_display_clipboard_get(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(62, 0, 1, callback);
...
function _godot_js_display_clipboard_set(p_text) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(63, 0, 1, p_text);
...
function _godot_js_display_cursor_lock_set(p_lock) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(66, 0, 1, p_lock);
 if (p_lock) {
...
function _godot_js_display_cursor_set_custom_shape(p_shape, p_ptr, p_len, p_hotspot_x, p_hotspot_y) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(67, 0, 1, p_shape, p_ptr, p_len, p_hotspot_x, p_hotspot_y);
...
function _godot_js_display_cursor_set_shape(p_string) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(68, 0, 1, p_string);
 GodotDisplayCursor.set_shape(GodotRuntime.parseString(p_string));
}

function _godot_js_display_cursor_set_visible(p_visible) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 0, 1, p_visible);
...
function _godot_js_display_fullscreen_cb(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(71, 0, 1, callback);
 const canvas = GodotConfig.canvas;
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_display_has_webgl(p_version) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 0, 1, p_version);
...
function _godot_js_display_notification_cb(callback, p_enter, p_exit, p_in, p_out) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(76, 0, 1, callback, p_enter, p_exit, p_in, p_out);
 const canvas = GodotConfig.canvas;
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_display_screen_size_get(width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(79, 0, 1, width, height);
 const scale = GodotDisplayScreen.getPixelRatio();
...
function _godot_js_display_setup_canvas(p_width, p_height, p_fullscreen, p_hidpi) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(80, 0, 1, p_width, p_height, p_fullscreen, p_hidpi);
 const canvas = GodotConfig.canvas;
...
function _godot_js_display_size_update() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(81, 0, 1);
...
function _godot_js_display_vk_cb(p_input_cb) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(85, 0, 1, p_input_cb);
...
function _godot_js_display_vk_show(p_text, p_type, p_start, p_end) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(87, 0, 1, p_text, p_type, p_start, p_end);
 const text = GodotRuntime.parseString(p_text);
...
function _godot_js_display_window_blur_cb(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(88, 0, 1, callback);
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_display_window_icon_set(p_ptr, p_len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(89, 0, 1, p_ptr, p_len);
...
function _godot_js_display_window_size_get(p_width, p_height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(90, 0, 1, p_width, p_height);
 GodotRuntime.setHeapValue(p_width, GodotConfig.canvas.width, "i32");
 GodotRuntime.setHeapValue(p_height, GodotConfig.canvas.height, "i32");
}

function _godot_js_display_window_title_set(p_data) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(91, 0, 1, p_data);
...
function _godot_js_fetch_create(p_method, p_url, p_headers, p_headers_size, p_body, p_body_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(92, 0, 1, p_method, p_url, p_headers, p_headers_size, p_body, p_body_size);
...
function _godot_js_fetch_free(id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(93, 0, 1, id);
 GodotFetch.free(id);
}

function _godot_js_fetch_http_status_get(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(94, 0, 1, p_id);
...
function _godot_js_fetch_read_chunk(p_id, p_buf, p_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(96, 0, 1, p_id, p_buf, p_buf_size);
 const obj = IDHandler.get(p_id);
 if (!obj || !obj.response) {
  return 0;
 }
...
function _godot_js_fetch_read_headers(p_id, p_parse_cb, p_ref) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(97, 0, 1, p_id, p_parse_cb, p_ref);
 const obj = IDHandler.get(p_id);
 if (!obj || !obj.response) {
  return 1;
 }
...
function _godot_js_fetch_state_get(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(98, 0, 1, p_id);
 const obj = IDHandler.get(p_id);
 if (!obj) {
  return -1;
 }
...
function _godot_js_input_drop_files_cb(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 0, 1, callback);
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_input_gamepad_cb(change_cb) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(100, 0, 1, change_cb);
...
function _godot_js_input_gamepad_sample_get(p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(103, 0, 1, p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard);
...
function _godot_js_input_key_cb(callback, code, key) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(104, 0, 1, callback, code, key);
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_input_mouse_button_cb(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(105, 0, 1, callback);
 const func = GodotRuntime.get_func(callback);
 const canvas = GodotConfig.canvas;
...
function _godot_js_input_touch_cb(callback, ids, coords) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(109, 0, 1, callback, ids, coords);
 const func = GodotRuntime.get_func(callback);
 const canvas = GodotConfig.canvas;
...
function _godot_js_input_vibrate_handheld(p_duration_ms) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(110, 0, 1, p_duration_ms);
...
function _godot_js_os_download_buffer(p_ptr, p_size, p_name, p_mime) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(111, 0, 1, p_ptr, p_size, p_name, p_mime);
...
function _godot_js_os_execute(p_json) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(112, 0, 1, p_json);
...
function _godot_js_os_finish_async(p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(113, 0, 1, p_callback);
 const func = GodotRuntime.get_func(p_callback);
...
function _godot_js_os_fs_sync(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(115, 0, 1, callback);
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_os_has_feature(p_ftr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(116, 0, 1, p_ftr);
...
function _godot_js_os_hw_concurrency_get() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(117, 0, 1);
 const concurrency = navigator.hardwareConcurrency || 1;
 return concurrency < 2 ? concurrency : 2;
}

function _godot_js_os_request_quit_cb(p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(118, 0, 1, p_callback);
 GodotOS.request_quit = GodotRuntime.get_func(p_callback);
}

function _godot_js_os_shell_open(p_uri) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(119, 0, 1, p_uri);
...
function _godot_js_pwa_cb(p_update_cb) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(120, 0, 1, p_update_cb);
...
function _godot_js_rtc_datachannel_close(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(122, 0, 1, p_id);
 const ref = IDHandler.get(p_id);
 if (!ref) {
  return;
 }
...
function _godot_js_rtc_datachannel_connect(p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(123, 0, 1, p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close);
...
function _godot_js_rtc_datachannel_get_buffered_amount(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(125, 0, 1, p_id);
...
function _godot_js_rtc_datachannel_send(p_id, p_buffer, p_length, p_raw) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(133, 0, 1, p_id, p_buffer, p_length, p_raw);
 const ref = IDHandler.get(p_id);
 if (!ref) {
  return 1;
 }
...
function _godot_js_rtc_pc_create(p_config, p_ref, p_on_connection_state_change, p_on_ice_gathering_state_change, p_on_signaling_state_change, p_on_ice_candidate, p_on_datachannel) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(135, 0, 1, p_config, p_ref, p_on_connection_state_change, p_on_ice_gathering_state_change, p_on_signaling_state_change, p_on_ice_candidate, p_on_datachannel);
...
function _godot_js_rtc_pc_datachannel_create(p_id, p_label, p_config) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(136, 0, 1, p_id, p_label, p_config);
 try {
  const ref = IDHandler.get(p_id);
  if (!ref) {
   return 0;
...
function _godot_js_rtc_pc_ice_candidate_add(p_id, p_mid_name, p_mline_idx, p_sdp) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(138, 0, 1, p_id, p_mid_name, p_mline_idx, p_sdp);
 const ref = IDHandler.get(p_id);
 if (!ref) {
  return;
 }
...
function _godot_js_rtc_pc_local_description_set(p_id, p_type, p_sdp, p_obj, p_on_error) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(139, 0, 1, p_id, p_type, p_sdp, p_obj, p_on_error);
 const ref = IDHandler.get(p_id);
 if (!ref) {
  return;
 }
...
function _godot_js_rtc_pc_offer_create(p_id, p_obj, p_on_session, p_on_error) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(140, 0, 1, p_id, p_obj, p_on_session, p_on_error);
 const ref = IDHandler.get(p_id);
 if (!ref) {
  return;
 }
...
function _godot_js_rtc_pc_remote_description_set(p_id, p_type, p_sdp, p_obj, p_session_created, p_on_error) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(141, 0, 1, p_id, p_type, p_sdp, p_obj, p_session_created, p_on_error);
 const ref = IDHandler.get(p_id);
 if (!ref) {
  return;
 }
...
function _godot_js_tts_get_voices(p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(142, 0, 1, p_callback);
 const func = GodotRuntime.get_func(p_callback);
 try {
  const arr = [];
...
function _godot_js_tts_is_paused() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(143, 0, 1);
 return window.speechSynthesis.paused;
}

function _godot_js_tts_is_speaking() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(144, 0, 1);
 return window.speechSynthesis.speaking;
}

function _godot_js_tts_pause() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(145, 0, 1);
 window.speechSynthesis.pause();
}

function _godot_js_tts_resume() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(146, 0, 1);
 window.speechSynthesis.resume();
}

function _godot_js_tts_speak(p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(147, 0, 1, p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback);
 const func = GodotRuntime.get_func(p_callback);
...
function _godot_js_websocket_close(p_id, p_code, p_reason) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(150, 0, 1, p_id, p_code, p_reason);
 const code = p_code;
...
function _godot_js_websocket_create(p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(151, 0, 1, p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close);
...
function _godot_js_websocket_send(p_id, p_buf, p_buf_len, p_raw) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(153, 0, 1, p_id, p_buf, p_buf_len, p_raw);
...
function _godot_js_wrapper_create_cb(p_ref, p_func) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(154, 0, 1, p_ref, p_func);
...
function _godot_js_wrapper_create_object(p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(155, 0, 1, p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
...
function _godot_js_wrapper_interface_get(p_name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(156, 0, 1, p_name);
 const name = GodotRuntime.parseString(p_name);
 if (typeof (window[name]) !== "undefined") {
...
function _godot_js_wrapper_object_call(p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(157, 0, 1, p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
...
function _godot_js_wrapper_object_get(p_id, p_exchange, p_prop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(158, 0, 1, p_id, p_exchange, p_prop);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return 0;
 }
...
function _godot_js_wrapper_object_getvar(p_id, p_type, p_exchange) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(159, 0, 1, p_id, p_type, p_exchange);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return -1;
 }
...
function _godot_js_wrapper_object_set(p_id, p_name, p_type, p_exchange) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(160, 0, 1, p_id, p_name, p_type, p_exchange);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return;
 }
...
function _godot_js_wrapper_object_set_cb_ret(p_val_type, p_val_ex) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(161, 0, 1, p_val_type, p_val_ex);
...
function _godot_js_wrapper_object_setvar(p_id, p_key_type, p_key_ex, p_val_type, p_val_ex) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(162, 0, 1, p_id, p_key_type, p_key_ex, p_val_type, p_val_ex);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return -1;
 }
...
function _godot_js_wrapper_object_unref(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(163, 0, 1, p_id);
...
function _godot_webgl2_glFramebufferTextureMultiviewOVR(target, attachment, texture, level, base_view_index, num_views) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(164, 0, 1, target, attachment, texture, level, base_view_index, num_views);
...
function _godot_webgl2_glGetBufferSubData(target, offset, size, data) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(165, 0, 1, target, offset, size, data);
 const gl_context_handle = _emscripten_webgl_get_current_context();
 const gl = GL.getContext(gl_context_handle);
 if (gl) {
...
function _godot_webxr_get_bounds_geometry(r_points) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(166, 0, 1, r_points);
...
function _godot_webxr_get_frame_rate() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(169, 0, 1);
...
function _godot_webxr_get_projection_for_view(p_view, r_transform) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(170, 0, 1, p_view, r_transform);
...
function _godot_webxr_get_render_target_size(r_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(171, 0, 1, r_size);
 const subimage = GodotWebXR.getSubImage();
 if (subimage === null) {
  return false;
 }
...
function _godot_webxr_get_supported_frame_rates(r_frame_rates) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(172, 0, 1, r_frame_rates);
...
function _godot_webxr_get_transform_for_view(p_view, r_transform) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(173, 0, 1, p_view, r_transform);
 if (!GodotWebXR.session || !GodotWebXR.pose) {
  return false;
 }
...
function _godot_webxr_get_visibility_state() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(176, 0, 1);
...
var _godot_webxr_initialize = function(p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(177, 0, 1, p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event);
...
function _godot_webxr_is_session_supported(p_session_mode, p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(178, 0, 1, p_session_mode, p_callback);
 const session_mode = GodotRuntime.parseString(p_session_mode);
...
function _godot_webxr_is_supported() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(179, 0, 1);
 return !!navigator.xr;
}
...
function _godot_webxr_update_input_source(p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(181, 0, 1, p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes);
...
function _godot_webxr_update_target_frame_rate(p_frame_rate) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(182, 0, 1, p_frame_rate);
...
var dynCall_jiiiiiii = Module["dynCall_jiiiiiii"] = createExportWrapper("dynCall_jiiiiiii", 8);

var dynCall_vijiiiiiiiii = Module["dynCall_vijiiiiiiiii"] = createExportWrapper("dynCall_vijiiiiiiiii", 13);

var dynCall_vijj = Module["dynCall_vijj"] = createExportWrapper("dynCall_vijj", 6);

var dynCall_vijiiiffi = Module["dynCall_vijiiiffi"] = createExportWrapper("dynCall_vijiiiffi", 10);

var dynCall_vijiiifii = Module["dynCall_vijiiifii"] = createExportWrapper("dynCall_vijiiifii", 10);
...
var dynCall_vijif = Module["dynCall_vijif"] = createExportWrapper("dynCall_vijif", 6);

var dynCall_vijifff = Module["dynCall_vijifff"] = createExportWrapper("dynCall_vijifff", 8);

var dynCall_viij = Module["dynCall_viij"] = createExportWrapper("dynCall_viij", 5);

var dynCall_fiji = Module["dynCall_fiji"] = createExportWrapper("dynCall_fiji", 5);

var dynCall_jij = Module["dynCall_jij"] = createExportWrapper("dynCall_jij", 4);

var dynCall_vijiiffifffi = Module["dynCall_vijiiffifffi"] = createExportWrapper("dynCall_vijiiffifffi", 13);

var dynCall_vijf = Module["dynCall_vijf"] = createExportWrapper("dynCall_vijf", 5);

var dynCall_fij = Module["dynCall_fij"] = createExportWrapper("dynCall_fij", 4);
...
var dynCall_iijjfj = Module["dynCall_iijjfj"] = createExportWrapper("dynCall_iijjfj", 9);

var dynCall_vijd = Module["dynCall_vijd"] = createExportWrapper("dynCall_vijd", 5);

var dynCall_vijid = Module["dynCall_vijid"] = createExportWrapper("dynCall_vijid", 6);

var dynCall_vijiiiii = Module["dynCall_vijiiiii"] = createExportWrapper("dynCall_vijiiiii", 9);

var dynCall_vijij = Module["dynCall_vijij"] = createExportWrapper("dynCall_vijij", 7);

var dynCall_jiji = Module["dynCall_jiji"] = createExportWrapper("dynCall_jiji", 5);

var dynCall_vijiiiiiii = Module["dynCall_vijiiiiiii"] = createExportWrapper("dynCall_vijiiiiiii", 11);

var dynCall_viiji = Module["dynCall_viiji"] = createExportWrapper("dynCall_viiji", 6);

var dynCall_viijfii = Module["dynCall_viijfii"] = createExportWrapper("dynCall_viijfii", 8);

var dynCall_viijii = Module["dynCall_viijii"] = createExportWrapper("dynCall_viijii", 7);

var dynCall_vijiii = Module["dynCall_vijiii"] = createExportWrapper("dynCall_vijiii", 7);

var dynCall_viiiiiiiiiiijjjjjjifiiiiii = Module["dynCall_viiiiiiiiiiijjjjjjifiiiiii"] = createExportWrapper("dynCall_viiiiiiiiiiijjjjjjifiiiiii", 32);
...
var dynCall_viijd = Module["dynCall_viijd"] = createExportWrapper("dynCall_viijd", 6);

var dynCall_diij = Module["dynCall_diij"] = createExportWrapper("dynCall_diij", 5);
...
var dynCall_jiijjj = Module["dynCall_jiijjj"] = createExportWrapper("dynCall_jiijjj", 9);

var dynCall_jiijj = Module["dynCall_jiijj"] = createExportWrapper("dynCall_jiijj", 7);

var dynCall_iiij = Module["dynCall_iiij"] = createExportWrapper("dynCall_iiij", 5);

var dynCall_viiijiji = Module["dynCall_viiijiji"] = createExportWrapper("dynCall_viiijiji", 10);

var dynCall_viiijjiji = Module["dynCall_viiijjiji"] = createExportWrapper("dynCall_viiijjiji", 12);

var dynCall_viijj = Module["dynCall_viijj"] = createExportWrapper("dynCall_viijj", 7);

var dynCall_viijiji = Module["dynCall_viijiji"] = createExportWrapper("dynCall_viijiji", 9);

var dynCall_iiiiijiii = Module["dynCall_iiiiijiii"] = createExportWrapper("dynCall_iiiiijiii", 10);

var dynCall_iiiiiijd = Module["dynCall_iiiiiijd"] = createExportWrapper("dynCall_iiiiiijd", 9);

var dynCall_diidj = Module["dynCall_diidj"] = createExportWrapper("dynCall_diidj", 6);

var dynCall_viiiijij = Module["dynCall_viiiijij"] = createExportWrapper("dynCall_viiiijij", 10);

var dynCall_viiidjj = Module["dynCall_viiidjj"] = createExportWrapper("dynCall_viiidjj", 9);

var dynCall_viidj = Module["dynCall_viidj"] = createExportWrapper("dynCall_viidj", 6);

var dynCall_iiijj = Module["dynCall_iiijj"] = createExportWrapper("dynCall_iiijj", 7);

var dynCall_jiid = Module["dynCall_jiid"] = createExportWrapper("dynCall_jiid", 4);

var dynCall_jiij = Module["dynCall_jiij"] = createExportWrapper("dynCall_jiij", 5);

var dynCall_viiiiddji = Module["dynCall_viiiiddji"] = createExportWrapper("dynCall_viiiiddji", 10);

var dynCall_iiiijf = Module["dynCall_iiiijf"] = createExportWrapper("dynCall_iiiijf", 7);

var dynCall_iiiiiiji = Module["dynCall_iiiiiiji"] = createExportWrapper("dynCall_iiiiiiji", 9);

var dynCall_jiiiii = Module["dynCall_jiiiii"] = createExportWrapper("dynCall_jiiiii", 6);

var dynCall_vijiji = Module["dynCall_vijiji"] = createExportWrapper("dynCall_vijiji", 8);
...
var dynCall_vijiff = Module["dynCall_vijiff"] = createExportWrapper("dynCall_vijiff", 7);

var dynCall_vijff = Module["dynCall_vijff"] = createExportWrapper("dynCall_vijff", 6);

var dynCall_vijjjj = Module["dynCall_vijjjj"] = createExportWrapper("dynCall_vijjjj", 10);

var dynCall_viiiiifiijii = Module["dynCall_viiiiifiijii"] = createExportWrapper("dynCall_viiiiifiijii", 13);

var dynCall_viiiiifiiijjii = Module["dynCall_viiiiifiiijjii"] = createExportWrapper("dynCall_viiiiifiiijjii", 16);

var dynCall_viiiiifiiijii = Module["dynCall_viiiiifiiijii"] = createExportWrapper("dynCall_viiiiifiiijii", 14);

var dynCall_viiiiifiiiijjii = Module["dynCall_viiiiifiiiijjii"] = createExportWrapper("dynCall_viiiiifiiiijjii", 17);

var dynCall_viijiiiiiiiii = Module["dynCall_viijiiiiiiiii"] = createExportWrapper("dynCall_viijiiiiiiiii", 14);
...
var dynCall_jiifff = Module["dynCall_jiifff"] = createExportWrapper("dynCall_jiifff", 6);

var dynCall_jiiifi = Module["dynCall_jiiifi"] = createExportWrapper("dynCall_jiiifi", 6);

var dynCall_jiiifiiiii = Module["dynCall_jiiifiiiii"] = createExportWrapper("dynCall_jiiifiiiii", 10);

var dynCall_viiiifijii = Module["dynCall_viiiifijii"] = createExportWrapper("dynCall_viiiifijii", 11);

var dynCall_viiiifiijjii = Module["dynCall_viiiifiijjii"] = createExportWrapper("dynCall_viiiifiijjii", 14);

var dynCall_vijiiifiijii = Module["dynCall_vijiiifiijii"] = createExportWrapper("dynCall_vijiiifiijii", 14);

var dynCall_vijiiifiiijjii = Module["dynCall_vijiiifiiijjii"] = createExportWrapper("dynCall_vijiiifiiijjii", 17);

var dynCall_vijiiifiiijii = Module["dynCall_vijiiifiiijii"] = createExportWrapper("dynCall_vijiiifiiijii", 15);

var dynCall_vijiiifiiiijjii = Module["dynCall_vijiiifiiiijjii"] = createExportWrapper("dynCall_vijiiifiiiijjii", 18);

var dynCall_fijiiii = Module["dynCall_fijiiii"] = createExportWrapper("dynCall_fijiiii", 8);

var dynCall_fijiiiii = Module["dynCall_fijiiiii"] = createExportWrapper("dynCall_fijiiiii", 9);

var dynCall_viiiiiiij = Module["dynCall_viiiiiiij"] = createExportWrapper("dynCall_viiiiiiij", 10);
...
var dynCall_iijiijiiiii = Module["dynCall_iijiijiiiii"] = createExportWrapper("dynCall_iijiijiiiii", 13);

var dynCall_vijjii = Module["dynCall_vijjii"] = createExportWrapper("dynCall_vijjii", 8);

var dynCall_vijiif = Module["dynCall_vijiif"] = createExportWrapper("dynCall_vijiif", 7);

var dynCall_iijijiiiii = Module["dynCall_iijijiiiii"] = createExportWrapper("dynCall_iijijiiiii", 12);

var dynCall_vijijj = Module["dynCall_vijijj"] = createExportWrapper("dynCall_vijijj", 9);

var dynCall_vijiiijj = Module["dynCall_vijiiijj"] = createExportWrapper("dynCall_vijiiijj", 11);

var dynCall_vijiijj = Module["dynCall_vijiijj"] = createExportWrapper("dynCall_vijiijj", 10);

var dynCall_vijjiji = Module["dynCall_vijjiji"] = createExportWrapper("dynCall_vijjiji", 10);

var dynCall_vijjiijii = Module["dynCall_vijjiijii"] = createExportWrapper("dynCall_vijjiijii", 12);

var dynCall_fijii = Module["dynCall_fijii"] = createExportWrapper("dynCall_fijii", 6);

var dynCall_iiiiiiij = Module["dynCall_iiiiiiij"] = createExportWrapper("dynCall_iiiiiiij", 9);

var dynCall_vijiiiij = Module["dynCall_vijiiiij"] = createExportWrapper("dynCall_vijiiiij", 10);

var dynCall_jijj = Module["dynCall_jijj"] = createExportWrapper("dynCall_jijj", 6);

var dynCall_jiiif = Module["dynCall_jiiif"] = createExportWrapper("dynCall_jiiif", 5);

var dynCall_vijiiiiiidddd = Module["dynCall_vijiiiiiidddd"] = createExportWrapper("dynCall_vijiiiiiidddd", 14);

var dynCall_jiiii = Module["dynCall_jiiii"] = createExportWrapper("dynCall_jiiii", 5);

var dynCall_jiijiiii = Module["dynCall_jiijiiii"] = createExportWrapper("dynCall_jiijiiii", 9);
...
var dynCall_jiijii = Module["dynCall_jiijii"] = createExportWrapper("dynCall_jiijii", 7);

var dynCall_iijiiij = Module["dynCall_iijiiij"] = createExportWrapper("dynCall_iijiiij", 9);

var dynCall_jijjjiiiiijii = Module["dynCall_jijjjiiiiijii"] = createExportWrapper("dynCall_jijjjiiiiijii", 17);

var dynCall_jijiiiiifiii = Module["dynCall_jijiiiiifiii"] = createExportWrapper("dynCall_jijiiiiifiii", 13);

var dynCall_viijiiiiiifiii = Module["dynCall_viijiiiiiifiii"] = createExportWrapper("dynCall_viijiiiiiifiii", 15);

var dynCall_vijfff = Module["dynCall_vijfff"] = createExportWrapper("dynCall_vijfff", 7);

var dynCall_vijfiff = Module["dynCall_vijfiff"] = createExportWrapper("dynCall_vijfiff", 8);

var dynCall_vijfi = Module["dynCall_vijfi"] = createExportWrapper("dynCall_vijfi", 6);

var dynCall_vijffffi = Module["dynCall_vijffffi"] = createExportWrapper("dynCall_vijffffi", 9);

var dynCall_vijiiffi = Module["dynCall_vijiiffi"] = createExportWrapper("dynCall_vijiiffi", 9);

var dynCall_vijiifffffff = Module["dynCall_vijiifffffff"] = createExportWrapper("dynCall_vijiifffffff", 13);

var dynCall_vijifiifffffifff = Module["dynCall_vijifiifffffifff"] = createExportWrapper("dynCall_vijifiifffffifff", 17);

var dynCall_vijiiffffiffffj = Module["dynCall_vijiiffffiffffj"] = createExportWrapper("dynCall_vijiiffffiffffj", 17);

var dynCall_vijiifff = Module["dynCall_vijiifff"] = createExportWrapper("dynCall_vijiifff", 9);

var dynCall_vijiffffffff = Module["dynCall_vijiffffffff"] = createExportWrapper("dynCall_vijiffffffff", 13);

var dynCall_vijiffff = Module["dynCall_vijiffff"] = createExportWrapper("dynCall_vijiffff", 9);

var dynCall_vijiifiififff = Module["dynCall_vijiifiififff"] = createExportWrapper("dynCall_vijiifiififff", 14);

var dynCall_vijifffij = Module["dynCall_vijifffij"] = createExportWrapper("dynCall_vijifffij", 11);

var dynCall_viijjjiifjii = Module["dynCall_viijjjiifjii"] = createExportWrapper("dynCall_viijjjiifjii", 16);

var dynCall_vijjjii = Module["dynCall_vijjjii"] = createExportWrapper("dynCall_vijjjii", 10);

var dynCall_vijiffifff = Module["dynCall_vijiffifff"] = createExportWrapper("dynCall_vijiffifff", 11);

var dynCall_vijjf = Module["dynCall_vijjf"] = createExportWrapper("dynCall_vijjf", 7);

var dynCall_vijiiifi = Module["dynCall_vijiiifi"] = createExportWrapper("dynCall_vijiiifi", 9);

var dynCall_vijiifi = Module["dynCall_vijiifi"] = createExportWrapper("dynCall_vijiifi", 8);

var dynCall_vijifi = Module["dynCall_vijifi"] = createExportWrapper("dynCall_vijifi", 7);

var dynCall_vijijiii = Module["dynCall_vijijiii"] = createExportWrapper("dynCall_vijijiii", 10);

var dynCall_vijijiiii = Module["dynCall_vijijiiii"] = createExportWrapper("dynCall_vijijiiii", 11);

var dynCall_vijijiiiff = Module["dynCall_vijijiiiff"] = createExportWrapper("dynCall_vijijiiiff", 12);

var dynCall_vijijii = Module["dynCall_vijijii"] = createExportWrapper("dynCall_vijijii", 9);

var dynCall_vijiijiiiiii = Module["dynCall_vijiijiiiiii"] = createExportWrapper("dynCall_vijiijiiiiii", 14);

var dynCall_vijiiij = Module["dynCall_vijiiij"] = createExportWrapper("dynCall_vijiiij", 9);

var dynCall_vijiiiiiiji = Module["dynCall_vijiiiiiiji"] = createExportWrapper("dynCall_vijiiiiiiji", 13);

var dynCall_vijjj = Module["dynCall_vijjj"] = createExportWrapper("dynCall_vijjj", 8);

var dynCall_vijdddd = Module["dynCall_vijdddd"] = createExportWrapper("dynCall_vijdddd", 8);

var dynCall_vijififi = Module["dynCall_vijififi"] = createExportWrapper("dynCall_vijififi", 9);

var dynCall_dij = Module["dynCall_dij"] = createExportWrapper("dynCall_dij", 4);

var dynCall_jiiiiiiiiii = Module["dynCall_jiiiiiiiiii"] = createExportWrapper("dynCall_jiiiiiiiiii", 11);
...
var dynCall_fijj = Module["dynCall_fijj"] = createExportWrapper("dynCall_fijj", 6);
